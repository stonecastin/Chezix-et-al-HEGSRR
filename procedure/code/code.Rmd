...
####Code block 0 - Functions and libraries
...
packages <- c(
  "here", "plyr", "tidyverse", "MARSS", "zoo", "lubridate", "RColorBrewer", "doParallel", "nlme", "ggplot2", "tidyr", "forecast", "stringr", "wesanderson", "lme4", "sf", "raster", "rgdal", "AICcmodavg", "dplyr"
)
...
writeLines(
  ...
  here::here("procedure", "environment", paste0("r-environment-", Sys.Date(), ".txt"))
  )
```
```{r}
library(dplyr)
...
#Load in libraries.
library(plyr);library(tidyverse);library(zoo);library(lubridate);library(here)
```

###Subset and Constrain Data

```{r, echo=FALSE, message=F, dependson=1}
load(here::here("data","raw","public","01_Data-Flow-Orig.RData"))
...
Data_Clean = plyr::match_df(Data,Stations.Final,on = "Station.ID")
save(Data_Clean,file=here::here("data", "derived", "private","02a_Data_Clean.RData"))
```
...
load(here::here("data", "derived", "private", "02a_Data_Clean.RData")) #Load processesed data.
...
save(Data.Preds, file = here::here("data", "derived", "private", "02b_Missing_Data_Predictions.RData"))
```
...
```{r,echo=F,message=F,dependson=1}
#Load Data
load(here::here("data", "raw", "public","01_Data-Wtshd-Orig.RData")); names(WS.2) = c("Station.ID","Lat","Long","Area")
load(here::here("data", "derived", "private","02b_Missing_Data_Predictions.RData"))
...
names(Y.Data)[8] = "DOY2"
save(Y.Data,file = here::here("data", "derived", "private","03_Data_Annual.RData"))
```
...
save(M.Data,file = here::here("data", "derived", "private","03_Data_Monthly.RData"))
```
...
load(here::here("data", "derived", "private","03_Data_Annual.RData"))
load(here::here("data", "derived", "private","03_Data_Monthly.RData"))
...
null_sim <- function(flow.dat, response, iter, capture_slopes_i = -999) {
  ...
  sim_varexp <- plyr::ldply(seq_len(iter), function(i) {
    ...
    if (i %in% capture_slopes_i) {
    	saveRDS(simulated_slopes, file =here::here("data", "results", "figures", paste0("sim-slopes-", response, "-", i, ".rds")))
    }
...
#These are some underlying real time series causing this: (already removed)
...
####Code block 2 - Climate Portfolio #REMOVE 1B OUTLIERS CODE BLOCK
...
```{r}
setwd(here::here("data","raw","private","BC_Basins_GoogleMapPL"))

BCwatersheds <- readOGR(dsn="BC_Basins_GoogleMapPL.shp", layer = "BC_Basins_GoogleMapPL", p4s = c("+init=epsg:3005"))
grid<-(BCwatersheds$BASIN = "FRASER")

```

#Extract elevation data from the clipped grid dataset. (UNKNOWN FILE)maybe just don't run it
...
#Loop through all the climate data files. (NOT WORKING)
...
for(i in dir()){
	...
	system.time({
		...
		df = read_csv(i) %>%
			dplyr::select(Year, ID1, Latitude, Longitude, Elevation, MAT, MAP,PAS, EMT, EXT, contains("Tave"), contains("Tmax"), contains("Tmin"), contains("PPT"), contains("PAS"), -contains("_wt"),-contains("_sp"), -contains("_sm"), -contains("_at"))
		...
		df6 = df %>% select(id1, latitude, longitude, elevation) %>%
			...
			write_csv(paste("../trends_split/",i,sep = "")) #FIX WRITE
	})
...
#Gather Together all the sites trend results. (UNKNOWN FILE)

```{r}
df = dir() %>%
	lapply(read_csv) %>%
	bind_rows()
write_rds(x = df, path = "../climate_trends.rds") #FIX WRITE LOCATION

df = read_rds(path  = "../climate_trends.rds") %>% data.frame() #FIX READ LOCATION
attr = names(df)[5:69]
write.table(attr, "../trends/attributes.txt", row.names = F, col.names = F, quote = F) #FIX WRITE LOCATION
```

#For QGIS viewing and raster creation. Remember I've multiplied the trend by 10^7 to retain the precision of the trend estimates as they are veryclose together for some variables. (MISSING FILES FROM ABOVE COMMNDS)
...
writeOGR(obj=sp, dsn="../../ClimateTrends", layer="trends", driver="ESRI Shapefile", overwrite_layer = T, morphToESRI = T) #FIX WRITE LOCATION
```

#Produce individual pour points for each Fraser sub-watershed.
#Three pour points need to be manually moved in Whitebox as the vector estimtes of flow path for these three sites are dissimilar enough that they will not be properly snapped to proper river. These sites are... 08KH006, 08LD001 and 08LE031. (CANNOT OPEN DATA SOURCE)
...
#Dissolve Polygon Features in Watersheds Shapefiles to One Single Polygon Feature. (this one works???)
...
#Combine all the watershed delineations into a single shapefile.(works?)
...
#Combine (ERROR SUBSCRIPT OUT OF BOUNDS)
...
writePolyShape(x = df, fn = "../watersheds") #FIX WRITE LOCATION
```
...
# with same order and drift (works, change where plots are saved)
...
out <- out %>% mutate(phi_re = (phi_sim - phi) / phi,
  sigma_re = (sigma_sim - sigma) / sigma) #it's one of these two outs at the beginning or end......
...
#Load/Organize Annual Data (works)

```{r}
load(here::here::here("data", "derived", "private","03_Data_Annual.RData"))
...
#Load/Orgnize Montly Data (works)
...
#Incorporate the Climate Data (works)
...
#Annual (ERROR SIM-STATS.CSV DOES NOT EXIST IN CODE.MD)
...
#Monthly (ERROR ATTEMPT TO SELECT 0 ITEMS FOR INTEGERONEINDEX)
...
#Gather Together all the sites timber harvest results. (data doesn't exist problems)
...
#Convert the 100m X 100m raster count data to area of timber harvest in km^2. (NON NUMBERIC ERGUMENT? MISSING DATA?)
...
save(Y.Data, file = here::here("data", "derived", "private","05_MonthDat_ClimForest.RData"))
save(M.Data, file = here::here("data", "derived", "private","05_AnnualDat_ClimForest.RData"))

load(here::here("data", "raw", "public","05_MonthDat_ClimForest.RData"))
load(here::here("data", "raw", "public","05_AnnualDat_ClimForest.RData"))
```
...
# time series (WITHOUT SLOPE) plus a gls() fit to that simulated data (will need to fix "out")
...
sim_slopes <- function(slope.dat, yrs, return_ts = FALSE){
	...
	if (return_ts) out <- out[,c("Station.ID", "yrs", "y")]
	out #again it's one of the two "out"s
}
```
...
# wrapper function for analysis of individual predictor variables. (need to fix "out")
...
# wrapper function to run analyses across all predictor variables. (need to fix "save" functions)
...
save(out_doy2, file = "out_doy2.RData") #here they are
...
aic_out <- aic_arima(Y.Data, "DOY2.logit") #prob need to fix out here
...
aic_out <- aic_arima(subset(M.Data, nMonth == 6), "log(med.log.sd)") #same w out here
aic_out %>% group_by(Station.ID) %>%
  mutate(delta_aic = aic_out - min(aic_out)) %>%
  ggplot(aes(paste(ar, ma), delta_aic, group = Station.ID)) + geom_line(alpha = 0.4)
```
...
#Funnel Plot Wrapper. (will prob need to fix "load"s)
...
load("out_doy2.RData") #here are the loads
load("out_min.RData")
load("out_max.RData")
load("out_med.RData")
...
density_plot = function(data, response, type, xlimit){
	...
	############################################ prepare data ###############################################
	#libraries to load. (may need to remove, same w above)
...
funnel.plots = function(dat, response, yaxis.doy = F, d.xaxis = F, pane, funnel.y = "", axis.ly = "", xaxis = F, ylimit = NULL, iter = 200, monthly = F, min = NA, max = NA){
	...
	#colour control panel
	col1="#f2ad00"; col2="#67a9cf"; col3="#f98400"; col4="#ff0000"; lcol="#969696"
	#libraries to load. (may need to remove)
	library(tidyverse);library(wesanderson);library(RColorBrewer)
	#cleanup and arrange observed flow and climate trend data.
	flow.trends = arrange.data(dat,response)
	#create a colour gradient for the climate index values.
...
load("max_month.rdata") #need to fix load here
...
load("min_month.rdata") #need to fix load
...
load("med_month.rdata") #need to fix load
...
#Attenuation Stats.
load("out_doy2.RData") #need tof fix loads here
load("out_med.RData")
load("out_min.RData")
load("out_max.RData")
library(tidyverse)
...
````{r}
        ...
        out = plyr::ldply(max_Areas, function(x){ #need to fix out
        ...
        ```{r}
        load("03_Data_Annual.RData") #need to fix load
        logit = function(p){log(p/(1-p))}
...
ggplot(for.plot, aes(year.center, pred.points*365, group = station.id))+
	geom_line(alpha = 0.2) +
	theme_classic() #where is this saving
```

#isolate the doy gls coefficients.

```{r}
varexp = out_doy2$Area$real_varexp$varexp #need to fix outs
...
load("max_month.RData") #need to fix loads
...
```{r}
load("out_med.RData") #need to fix load
...
load("05_MonthDat_ClimForest.RData") #need to fix loads, libraries?
load("05_AnnualDat_ClimForest.RData")
```

#S1 (fix ggsave and folder= for all of them)
...
```{r}
load("out_med.RData") #need to fix loads
...
```{r}
load("02b_Missing_Data_Predictions.RData") #fix load
```
